...
SCENARIO("Test whether the random Sensor value generator is generating expected numbder of values")
{
    GIVEN("The maximum value 50, minimum value 1 and maximum count of values to be generated 100")
    {
        ...
        WHEN("At the beginning during sensor value generation")
        {
            vector<int> generatedValueList=generateRandomSensorValues<int>(minValue,maxValue);
            THEN("Random value list of size 100 within given range(1-50) is generated")
            {
                REQUIRE(generatedValueList.size()==100);
                for(int i=0;i<100;i++)
	        {
		      REQUIRE(minValue<=generatedValueList[i]);
		      REQUIRE(generatedValueList[i]<=maxValue);
		}
            }
        }
    }
